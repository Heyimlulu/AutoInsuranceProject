	...
	public ResponseEntity<Message> getVehicleById(@PathVariable long id) {
		try {
			...
			if (optBill.isPresent()) {
				return new ResponseEntity<Message>(new Message("Successfully! Retrieve a Vehicle by id = " + id, null, null, null, Arrays.asList(optBill.get()), null, null, ""), HttpStatus.OK);
			} else {
		...
		try {
			if (vehicleServices.checkExistedVehicle(id)) {
				...
				return new ResponseEntity<Message>(
						new Message("Successfully! Updated a Vehicle " + "with id = " + id, null, null, null, null, null, null, ""), HttpStatus.OK);
			} else {
				return new ResponseEntity<Message>(
