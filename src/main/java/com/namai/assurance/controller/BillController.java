...
public class BillController {
	...
	public ResponseEntity<Message> addNewBill(@RequestBody Bill bill, @PathVariable long id) {
		try {
			...
			return new ResponseEntity<Message>(new Message("Upload Successfully!", null, null, Arrays.asList(returnedPolicy), null, null, null, null, ""), HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<Message> getBillById(@PathVariable long id) {
		try {
			...
			if (optPolicy.isPresent()) {
				return new ResponseEntity<Message>(new Message("Successfully! Retrieve a Bill by id = " + id, null, null, Arrays.asList(optPolicy.get()), null,null, null, null, ""), HttpStatus.OK);
			} else {
		...
		try {
			if (billServices.checkExistedBill(id)) {
				...
				return new ResponseEntity<Message>(
						new Message("Successfully! Updated a Bill " + "with id = " + id, null, null, null, null, null, null, ""), HttpStatus.OK);
			} else {
				return new ResponseEntity<Message>(
